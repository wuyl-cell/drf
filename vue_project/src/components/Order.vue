<template>
    <div class="cart">
        <Header/>
        <div class="cart-info">
            <h3 class="cart-top">购物车结算 <span>共{{this.$store.state.cart_length}}门课程</span></h3>
...
export default {
    ...
    methods: {
        ...
        get_select_course(){
            ...
            this.$axios({
                url: this.$settings.HOST + 'cart/order_info/',
                method: "get",
                headers: {
                    "Authorization": "jwt " + token
                }
        ...
        create_order(){
            let token = this.check_user_login()
            this.$axios({
                url: this.$settings.HOST + 'order/create_order/',
                method: "post",
                data: {
                    pay_type: 1
                },
